...
> Репозиторий **приватный** — для `wget`/`curl` нужен GitHub token с правом **Contents: Read**.

### Способ 1 — одна команда (с токеном)

```bash
export GITHUB_TOKEN="github_pat_ВАШ_ТОКЕН"
...
Repository: `WDTT-Cudy-TR3000-256mb`, Permission: **Contents: Read**

### Способ 2 — скопировать скрипт (без wget с GitHub на роутере)

На компьютере (где есть доступ к репозиторию):

```bash
git clone git@github.com:RSokolovRS/WDTT-Cudy-TR3000-256mb.git
scp WDTT-Cudy-TR3000-256mb/install.sh root@192.168.1.1:/tmp/
```
...
### Способ 3 — сохранить токен на роутере (для обновлений)

```bash
mkdir -p /etc/wdtt
echo 'github_pat_...' > /etc/wdtt/github_token
chmod 600 /etc/wdtt/github_token
sh /tmp/install.sh
```
