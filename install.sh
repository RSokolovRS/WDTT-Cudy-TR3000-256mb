...
WDTT_INSTALL_VERSION="3.12.1"
WDTT_ROUTING_VERSION="3.12.1"
WDTT_BIN_TAG="v3.8.3"
...
restore_wdtt_secrets() {
	...
	for v in peer password hashes enabled captcha_mode vk_auth_mode obfs_mode workers routing_mode uplink_iface; do
		[ -f "$f/$v" ] || continue
		[ -s "$f/$v" ] || continue
		val="$(cat "$f/$v")"
		if [ "$v" = "hashes" ]; then
